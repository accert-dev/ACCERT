Running the Combined IAT and CRT GUI Case
=========================================

The ACCERT GUI can run a connected International Adjustment Tool (IAT) and
Cost Reduction Framework Tool (CRT) workflow. In this workflow, IAT first converts a
U.S.-based ACCERT or CRT baseline CSV to the selected country. CRT then uses
...
* Set ``Mode`` to ``IAT then CRT``.
* Set ``Output name`` to a short name for the generated files, such as
  ``ap1000_china_gui``.

This workflow creates an IAT-adjusted CSV first, then passes that CSV to CRT
through the CRT ``baseline_csv`` setting.
...
* Set ``ACCERT CSV file`` to the input CSV path. This can be either a
  ...
  CRT/IAT baseline format before running IAT.
...
Set the CRT Fixed Inputs
------------------------

In the ``CRT Fixed Inputs`` panel:
...
In the ``CRT Levers`` panel, set the orderbook and reduction levers:
...
* ``Interest %`` sets the financing interest rate used by CRT.
...
* ``Standardization %``, ``Commercial BOP``, ``Non-safety-related RB``, and
  ``Modular civil constr.`` set the remaining CRT reduction levers.
...
* The IAT-adjusted CSV, which becomes the CRT baseline for this run.
* The CRT results CSV shown in the GUI results-table tab.
* The CRT dashboard PNG.
* Any additional CSV or image outputs linked in the result panel.

The original CSV in ``src/crt/data`` is not modified.
