                        ...
                        for  mod_0 in ["stick_built", "modularized"]:
                            for standardization_0 in [0.7, 0.95]:
                                for BOP_grade_0 in [ "nuclear", "non_nuclear"]:
                                    for RB_grade_0 in [  "nuclear", "non_nuclear"]:
                                        ...
                                        for n_th in range(1, num_orders+1):
                                            ...
                                            cons_duration_list.append(final_construction_duration)
                                        ...
                                        avg_duration = np.mean(cons_duration_list)

                                        OCC_duration_list.append(avg_OCC)
                                    
                                    
                                        TCI_duration_list.append(avg_TCI)
                                        duration_list.append(avg_duration)
                                        counter = counter  +1 
                                        print(counter )
# if reactor_type == "SFR":
g= sns.scatterplot(x=duration_list, y= OCC_duration_list, color = "blue", edgecolor = "black",  legend = False)
g.collections[0].set_sizes([100])                 # <---- reset markersize here

    #     g.set_xticks(range(1,22,2))
    # else:    
    #     g= sns.lineplot(x=orders_list, y=OCC_N_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='red') 
    #     g.set_xticks(range(1,22,2))

    # if reactor_type == "SFR":
    #     g= sns.lineplot(x=orders_list, y=TCI_N_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='blue') 
    #     g.set_xticks(range(1,22,2))
    # else:    
    #     g= sns.lineplot(x=orders_list, y=TCI_N_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='red') 
    #     g.set_xticks(range(1,22,2))

plt.xlabel('Fleet-Avg Construction Duration (months)', fontsize='25') # x-axis name
plt.ylabel('Fleet-Avg OCC (2022$/kWe)', fontsize='25') # x-axis name
...
plt.savefig('OCC_cons duration.png')
