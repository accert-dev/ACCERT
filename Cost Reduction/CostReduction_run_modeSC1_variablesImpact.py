...
reactor_type = "HTGR" # This can be SFR or HTGR
...
for scenario in ["best", "# Orders", "Design Completion", "AE Exp", "Construction Exp",\
                "Supply Chain Exp", "Design Maturity", "Standardization", "Modularization",\
                "Safety-Related BOP", "Safety-Related RB", "ITC", "Interest Rate", "Land Cost",\
                    "Startup Duration"]:

    print(scenario)
    if scenario == "best":
        num_orders = 15
        design_completion_0 = 1
        ae_exp_0 = 2
        ce_exp_0 = 2
        proc_exp_0 = 2
        Design_Maturity_0 = 2
    ...
    if scenario == "Safety-Related BOP":
        num_orders = 15
        design_completion_0 = 1
        ae_exp_0 = 2
        ce_exp_0 = 2
    ...
    if scenario == "Safety-Related Reactor Building":
        num_orders = 15
        design_completion_0 = 1
        ae_exp_0 = 2
        ce_exp_0 = 2
...
## ## User-defined Independent Variables (Global Levers)
        ...
        # OCC_list.append(ITC_reduced_OCC/reactor_power)
            TCI_list_1.append( ( (tot_cap_investment - tot_overnight_cost*ITC_cost_reduction_factor)/reactor_power )[0]  )

        elif n_th ==num_orders:
            TCI_list_n.append((tot_cap_investment - tot_overnight_cost*ITC_cost_reduction_factor)/reactor_power)

      
non_best_scenarios_1 = TCI_list_1[1:]
# non_best_scenarios_n = TCI_list_n[1:]

TCI_change_1 = (non_best_scenarios_1 -   TCI_list_1[0])

var_dict = {scenarios_list[1:][i]: TCI_change_1[i] for i in range(len(scenarios_list[1:]))}
# sorting the variables based on impact
var_dict_sorted = sorted(var_dict.items(), key=lambda x: x[1], reverse=True)    

var_dict_sorted_1 = list(zip(*var_dict_sorted))
# var_dict_new = sorted(var_dict.items(), key=lambda x: x[1])

# print(type(var_dict_new ))
# # print( list(var_dict_new.keys())) 
# print( list(var_dict_new.values()))    

# print(list(TCI_change_1))
# print(TCI_list[0])
# print(non_best_scenarios)
...
# #     # In[ ]:
xx = np.arange(len(non_best_scenarios_1)) 

# sns.barplot(x= scenarios_list[1:], y=list(TCI_change_1), order=list(TCI_change_1))

plt.bar( var_dict_sorted_1[0] , var_dict_sorted_1[1])
# # plt.bar(xx + 0.3 , TCI_change_n)
plt.xlabel('Levers', fontsize='16') # x-axis name
plt.ylabel('Impact on the FOAK TCI (2022$/kWe)', fontsize='16') # x-axis name


plt.tick_params(labelsize=16)
plt.xticks(rotation=45, ha="right")

plt.legend(["HTGR" ], fontsize='16')
plt.savefig('HTGR_leverImpact.png')
