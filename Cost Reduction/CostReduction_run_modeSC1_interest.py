...
## ## User-defined Independent Variables (Global Levers)
    ...
    TCI_interest_list = []
    interest_list = []
    num_orders = 10 # number of firm orders
    ...
    for interest_rate_0 in [x / 100.0 for x in range(3, 13)]:
        ...
        TCI_interest_list.append(avg_TCI)
        interest_list.append(interest_rate)
        print(interest_rate)
    # if reactor_type == "SFR":
    #     g= sns.lineplot(x=orders_list, y=OCC_N_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='blue') 
    ...
    if reactor_type == "SFR":
        g= sns.lineplot(x=100*np.array(interest_list), y=TCI_interest_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='blue') 
        # g.set_xticks(range(3,10,13))
    else:    
        g= sns.lineplot(x=100*np.array(interest_list), y=TCI_interest_list, linestyle='-', marker='o', markersize=8, label=reactor_type, color='red') 
        # g.set_xticks(range(3,10,13))

plt.xlabel('Interest Rate (%)', fontsize='25') # x-axis name
...
plt.legend(loc='upper left', fontsize='25') # Add a legend
plt.ylim(5000, 10000)
plt.tick_params(labelsize=25)

plt.savefig('ITC_interest.png')
