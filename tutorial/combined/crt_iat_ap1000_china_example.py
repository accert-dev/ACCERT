"""Run IAT on the AP1000 baseline, then use that China-adjusted CSV in CRT.

This example keeps the original AP1000 baseline in ``src/crt/data`` unchanged.
IAT writes a separate adjusted CSV into ``tutorial/combined``. CRT then uses
...
ORIGINAL_AP1000_BASELINE = REPO_ROOT / "src" / "crt" / "data" / "AP1000_baseline.csv"
IAT_OUTPUT_CSV = REPO_ROOT / "tutorial" / "combined" / "iat_ap1000_china_for_crt.csv"
DASHBOARD_OUTPUT = REPO_ROOT / "tutorial" / "combined" / "crt_iat_ap1000_china_dashboard.png"
...
if __name__ == "__main__":
    ...
    print("\nStep 2: Run CRT using the IAT-adjusted AP1000 baseline\n")
    print(f"CRT baseline_csv: {crt_config['baseline_csv']}\n")
    ...
    save_dashboard(
        crt_result,
        DASHBOARD_OUTPUT,
        title="AP1000 China Cost Reduction Framework Tool",
        show_levers=True,
    )
    print("\nOutputs:")
    print(f"IAT-adjusted CRT baseline CSV: {IAT_OUTPUT_CSV}")
    print(f"CRT dashboard figure: {DASHBOARD_OUTPUT}")
